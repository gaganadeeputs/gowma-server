...
public class UserRepository extends BaseRepository {
    ...
    private static final String INSERT_SQL = "INSERT INTO gowma_user(" +
            "gowma_user__email, gowma_user__mobile_no, gowma_user__password_hash," +
            "gowma_user__status, gowma_user__created_date ,gowma_user__created_by)" +
            "VALUES (:email, :mobileNo, :password, :status::status, :createdDate, :createdBy)";
    ...
    private static String SELECT_BY_ID_SQL = "SELECT * FROM gowma_user" +
            " WHERE id=:id" +
            " AND gowma_user__is_deleted=false";

    private static String FIND_BY_EMAIL_SQL = "SELECT * FROM gowma_user " +
            " WHERE gowma_user__email=:email" +
            " AND gowma_user__is_deleted=false";


    private static String FIND_BY_MOBILE_NO_SQL = "SELECT * FROM gowma_user" +
            " WHERE gowma_user__mobile_no=:mobileNo" +
            " AND gowma_user__is_deleted=false";
    ...
    public User get(int userId) {
        ...
        List<User> users = namedParameterJdbcTemplate.query(SELECT_BY_ID_SQL, paramNameToValuesMap,new UserMapper());
        if (users.isEmpty())
    ...
    public User findByEmail(String email) {
        ...
        List<User> users = namedParameterJdbcTemplate.query(FIND_BY_EMAIL_SQL, paramNameToValuesMap, new UserMapper());
        if (users.isEmpty())
            return null;
        return users.get(0);
    }
    ...
    public User findByMobileNumber(String mobileNumber) {
        ...
        List<User> users = namedParameterJdbcTemplate.query(FIND_BY_MOBILE_NO_SQL, paramNameToValuesMap, new UserMapper());
        if (users.isEmpty())
            return null;
        return users.get(0);
    }
    ...
    private class UserMapper implements RowMapper<User> {
        ...
        public User mapRow(ResultSet resultSet, int i) throws SQLException {
            ...
            List<Role> roles = userRoleMappingRepository.getByUserId(user.getId());
            user.setRoles(roles);
            return user;
        }
    }

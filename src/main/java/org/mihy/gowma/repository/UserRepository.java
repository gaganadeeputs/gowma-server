...
public class UserRepository extends BaseRepository {
    ...
    private static final String UPDATE_SQL = "UPDATE gowma_user" +
            "SET  gowma_user__email=:email, gowma_user__mobile_no=:mobileNo, gowma_user__password_hash=:password, gowma_user__status=:status" +
    ...
    public User create(final User user) {
        try {
            ...
            userDetailRepository.create(user.getUserDetail());
            user.getUserAddresses().forEach(userAddress -> userAddress.setUserId(user.getId()));
            userAddressRepository.create(user.getUserAddresses());
    ...
    public User get(int userId) {
        ...
        user.setUserAddresses(userAddressRepository.getByUserId(userId));
    ...
    public User update(User user) {
        try {
            namedParameterJdbcTemplate.update(UPDATE_SQL, new EnumBeanPropParamSource(user));
            user.getUserDetail().setUserId(user.getId());
            userDetailRepository.update(user.getUserDetail());
            user.getUserAddresses().forEach(userAddress -> userAddress.setUserId(user.getId()));
            userAddressRepository.update(user.getUserAddresses());
        } catch (DuplicateKeyException dke) {
            throw dke;
        }
        return user;
